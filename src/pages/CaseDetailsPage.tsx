...
const CaseDetailsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCasePayments = async () => {
      ...
      try {
        ...
        if (data) {
          // Convert database format to component format
          const formattedPayments: CasePayment[] = data.map((p: any) => ({
            id: p.id,
            amount: parseFloat(p.amount),
            date: new Date(p.date),
            receivedBy: p.received_by || 'Unknown',
          ...
          console.log('✅ Loaded', formattedPayments.length, 'payments for case');
        }
      } catch (err) {
        console.error('Error fetching case payments:', err);
      }
    };
  ...
  const handleAddPayment = async () => {
    ...
    if (!id) {
      setPaymentNotification({ type: 'error', message: 'Case ID not found' });
      setTimeout(() => setPaymentNotification(null), 3000);
      return;
    }
    ...
    try {
      // Save to database
      const { data, error } = await db.casePayments.create({
        case_id: id,
      ...
      if (error) {
        ...
        setPaymentNotification({ type: 'error', message: `Failed to save payment: ${error.message}` });
      ...
      setPaymentNotification({ type: 'error', message: `Failed to add payment: ${error.message || 'Unknown error'}` });
      setTimeout(() => setPaymentNotification(null), 3000);
    } finally {
  ...
  const handleAcceptPayment = async (paymentId: string) => {
    try {
      // Update in database
      const { data, error } = await db.casePayments.accept(paymentId, user?.name || 'Admin');
      
      if (error) {
        console.error('Error accepting payment:', error);
        setPaymentNotification({ type: 'error', message: `Failed to accept payment: ${error.message}` });
        setTimeout(() => setPaymentNotification(null), 5000);
        return;
      }
      ...
      setPaymentNotification({ type: 'success', message: 'Payment accepted by admin! Fees updated.' });
      setTimeout(() => setPaymentNotification(null), 3000);
      
      console.log('✅ Payment accepted:', data);
    } catch (error: any) {
      console.error('Error accepting payment:', error);
      setPaymentNotification({ type: 'error', message: 'Failed to accept payment' });
      setTimeout(() => setPaymentNotification(null), 3000);
    }
  };
